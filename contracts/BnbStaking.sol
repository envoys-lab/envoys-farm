...
import '@envoysvision/envoys-swap-lib/contracts/math/SafeMath.sol';
import '@envoysvision/envoys-swap-lib/contracts/token/BEP20/IBEP20.sol';
import '@envoysvision/envoys-swap-lib/contracts/token/BEP20/SafeBEP20.sol';
...
contract BnbStaking is Ownable {
    ...
    // The block number when EVT mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 evtReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
