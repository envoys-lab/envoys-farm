...
contract('SousChef', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.evb = await MockBEP20.new('LPToken', 'LP1', '1000000', {
      from: minter,
    });
    this.chef = await SousChef.new(this.evb.address, '40', '300', '400', {
      from: minter,
    });
  });

  it('sous chef now', async () => {
    await this.evb.transfer(bob, '1000', { from: minter });
    await this.evb.transfer(carol, '1000', { from: minter });
    await this.evb.transfer(alice, '1000', { from: minter });
    assert.equal((await this.evb.balanceOf(bob)).toString(), '1000');

    await this.evb.approve(this.chef.address, '1000', { from: bob });
    await this.evb.approve(this.chef.address, '1000', { from: alice });
    await this.evb.approve(this.chef.address, '1000', { from: carol });

    await this.chef.deposit('10', { from: bob });
    assert.equal(
      (await this.evb.balanceOf(this.chef.address)).toString(),
      '10'
    );
  ...
  it('try evb', async () => {
    ...
    this.chef2 = await SousChef.new(this.evb.address, '40', '600', '800', {
      from: minter,
    });
    await this.evb.approve(this.chef2.address, '10', { from: alice });
    await time.advanceBlockTo('590');
    this.chef2.deposit('10', { from: alice }); //520
    await time.advanceBlockTo('610');
    assert.equal(
      (await this.evb.balanceOf(this.chef2.address)).toString(),
      '10'
    );
    assert.equal(
  ...
  it('emergencyWithdraw', async () => {
    ...
    assert.equal((await this.evb.balanceOf(alice)).toString(), '990');
